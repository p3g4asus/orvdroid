...
public class ParcelableUtil {
    ...
    public static void marshallToSharedPref(Parcelable p, String key, SharedPreferences.Editor sharedEdit) {
        sharedEdit.putString(key,toStr(marshall(p)));
    }

    public static <T extends Parcelable> T unmarshallFromSharedPref(Parcelable.Creator<T> creator,String key,SharedPreferences sharedPreferences) {
        String v = sharedPreferences.getString(key,null);
        if (v==null)
            return null;
        else
            return unmarshall(fromStr(v),creator);
    }
